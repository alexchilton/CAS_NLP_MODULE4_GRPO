...
FINE_TUNE_PATH = os.getenv("HUGGINGFACE_MODEL_PATH", "fine_tune").strip()
...
# If fine-tune adapter exists, load it
adapter_config_path = os.path.join(FINE_TUNE_PATH, "adapter_config.json")
if os.path.exists(adapter_config_path):
    logger.info(f"Found PEFT adapter at {FINE_TUNE_PATH}, loading...")
    model = PeftModel.from_pretrained(model, FINE_TUNE_PATH)
    logger.info("Loaded model with PEFT adapter.")
else:
    logger.info("No PEFT adapter found, using base model only.")
...
# FIX: PROMPT CONTAMINATION - Removed STORM/BRAVE/BRISK examples
# These were memorized by the model during training, causing hallucinations
SYSTEM_PROMPT = """
...
def render_user_prompt(past_guesses: List[GuessWithFeedback]) -> str:
    """
    FIX: Enhanced prompt with explicit position tracking to prevent misinterpretation
    """
    ...
    if past_guesses:
        ...
        # FIX: Add explicit summary to prevent misinterpretation
...
def generate_stream(prompt: str) -> str:
    logger.info("Generating model output for prompt using model.generate.")
    ...
    # Remove the prompt from the output to get only the completion
...
def get_feedback(guess: str, secret_word: str) -> List[LetterFeedback]:
    ...
    # FIX: Proper Wordle feedback logic (handle duplicate letters correctly)
    guess = guess.upper()
