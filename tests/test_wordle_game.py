...
class TestValidateGuess:
    ...
    def test_duplicate_letters_guess(self):
        ...
        secret = "SPEED"  # S(0) P(1) E(2) E(3) D(4)
        guess = "EERIE"   # E(0) E(1) R(2) I(3) E(4)
        feedback = validate_guess(secret, guess)
        # First two E's misplaced (match SPEED's two E's), R and I wrong, last E wrong (no more E's)
        parts = feedback.split()
        assert "E(-)" in parts[0]  # First E misplaced (matches one E in SPEED)
        assert "E(-)" in parts[1]  # Second E misplaced (matches second E in SPEED)
        assert "R(x)" in parts[2]  # R not in SPEED
        assert "E(x)" in parts[4]  # Third E wrong (no more E's available in SPEED)
