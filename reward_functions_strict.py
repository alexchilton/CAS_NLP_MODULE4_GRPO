...
def output_format_check(prompt: str, completion: str, example: dict, training_progress: float = 0.0) -> float:
    ...
    Penalties:
    - Text after </guess>: -2.0 (prevents hallucination)
    ...
    try:
        ...
        # STRICT: Check for exact pattern with nothing after </guess>
        # Use $ to ensure nothing follows </guess>
        strict_regex = (
            r"^<think>\s*([^<]*(?:<(?!/?think>)[^<]*)*)\s*</think>\s*"
            r"<guess>\s*([\s\S]*?)\s*</guess>\s*$"
        )
        
        # First check if there's text after </guess> - HEAVY PENALTY
        if "</guess>" in completion:
            after_guess = completion.split("</guess>", 1)
            if len(after_guess) > 1 and after_guess[1].strip():
                # There is text after </guess> - this is hallucination!
                return -2.0
