...
def setup_model_and_tokenizer_peft():
    ...
    bnb_config = BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_quant_type="nf4",           # Best for neural weights
        bnb_4bit_compute_dtype=torch.float16, # Match 4070 capabilities
...
def run_train(train_dataset, val_dataset):
    ...
    training_args = GRPOConfig(
        output_dir="output5/wordle-grpo-rtx4070",
        num_train_epochs=1,
        ...
        learning_rate=1e-5,
        logging_steps=1,
        eval_strategy="no",          # or "steps" if you want periodic eval
        eval_steps=20,
        save_strategy="no",          # we're manually saving final/best
        save_steps=20,
        save_total_limit=1,
        bf16=True,
        fp16=False,
        ...
        logging_dir="output5/wordle-grpo-rtx4070/logs",
        report_to=["tensorboard", "wandb"],
        run_name=f"wordle-grpo-rtx4070-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}",
        temperature=0.7,
        top_p=0.9,
        top_k=50,
