"""
Test Base Qwen Model with Structured Prompts
=============================================

Tests the base Qwen/Qwen2.5-3B-Instruct model using the structured prompt
...
Usage:
    ...
    python test_base_qwen.py --games 50 --temperature 0.7 --seed 123
"""
    ...
    for turn in range(1, max_guesses + 1):
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                max_new_tokens=4096,  # Full context for Qwen-3B (verbose reasoning)
...
def main():
    parser = argparse.ArgumentParser(
        description="Test base Qwen model with structured prompts"
    )
    ...
    detailed_log_path = f"detailed_log_temp{args.temperature}_games{args.games}.txt"
    with open(detailed_log_path, 'w') as detailed_log:
        detailed_log.write(f"DETAILED LOG - Base Qwen Testing\n")
    ...
    # Save results
    output_file = f"results_base_qwen_temp{args.temperature}_games{args.games}.txt"
