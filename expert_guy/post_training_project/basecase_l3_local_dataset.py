...
# Load five_letter_words.csv, skip header
words_df = pd.read_csv('five_letter_words.csv', header=0)
# Assume the first column contains the words
if words_df.shape[1] > 1:
    words = words_df.iloc[:, 0].astype(str)
else:
    words = words_df.squeeze().astype(str)

# Filter for valid 5-letter alphabetic words only
words = words[words.str.len() == 5]
words = words[words.str.isalpha()]
words = words.str.upper()
logger.info(f"Loaded {len(words)} valid 5-letter words from local CSV.")

# Split into train/validation (80/20 split)
train_secrets, val_secrets = train_test_split(words, test_size=0.2, random_state=42)
...
load_dotenv()
MODEL_NAME = "Qwen/Qwen2.5-3B-Instruct"   # 3 B model
HF_TOKEN = None
